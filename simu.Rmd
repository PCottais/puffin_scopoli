...
moves <- function(steps){
  ...
  prob_avail <- sub/sum(sub)
  prob_avail <- prob_avail %>% as_vector()
  names(prob_avail) <- 1:9
  num_cell <- which(rmultinom(1, 1, prob_avail)==1)
  move <- code[num_cell,]
...
```{r}
heatmap(envi, Rowv = NA, Colv = NA, col = pal)
lines(envi_dt[coord$row, coord$col], lwd = 0.2)
points(envi_dt[coord$row, coord$col], lwd = 0.1, pch = 3, cex = 0.2)
```

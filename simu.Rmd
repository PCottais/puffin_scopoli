...
s <- 15                # number of spatial points estimate at each step
...
# give first h value
point <- samp[1,]
esti.l$hx[1] <- env_rho[point$row,point$col]
...
for (i in 2:n){
  #estimate h
  point <- samp[i,]
  esti.l$h[i] <- env_rho[point$row,point$col]
  
  #save h 
  if (i %in% 2:(n-1)){
  esti.l$hx[i+1] <- esti.l$h[i]
  }
